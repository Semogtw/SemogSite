...
function assertBatchSucceeded(results: readonly D1QueryResult[]): void {
  ...
  if (failed !== undefined) {
    throw new Error("D1 attention lifecycle batch failed.");
  }
}
...
function readChangeCount(result: D1QueryResult | undefined): number {
  ...
  if (typeof changes !== "number" || !Number.isInteger(changes) || changes < 0) {
    throw new Error("D1 attention lifecycle result is missing changes metadata.");
  }
  return changes;
}
  ...
  implements AttentionLifecycleRepository
{
  ...
  async transitionWithAudit(
    ...
    if (changed > 1) {
      throw new Error("D1 attention lifecycle CAS changed more than one row.");
    }
    return changed === 1;
  }
}
