...
describe("D1 cooperative run event pagination", () => {
  it("uses sequence as a bound immutable keyset cursor", async () => {
    ...
    expect(query?.sql).toContain("ORDER BY sequence DESC");
    expect(query?.params).toEqual(["cooperative-run-1", 50, 20]);
    expect(query?.sql).not.toContain("cooperative-run-1");
  });

  it("omits the sequence predicate on the first page", async () => {
    const binding = new CapturingD1();
    const model = new D1CooperativeRunReadModel(binding);

    await model.listEvents("cooperative-run-1", { limit: 100 });

    const query = binding.executed[0];
    expect(query?.sql).not.toContain("sequence < ?");
    expect(query?.params).toEqual(["cooperative-run-1", 100]);
  });
});
