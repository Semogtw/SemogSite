...
export class D1TodayDataSource implements TodayDataSource {
  ...
  private async listWorkByState(
    ...
    return Promise.all(
      rows.map(async (row): Promise<TodayWorkItem> => {
        const latestEvidence = await this.database
          .select({
  ...
  private async listAttentionByOwners(
    owners: readonly ("owner" | "shared" | "external_environment")[],
  ): Promise<readonly TodayAttentionItem[]> {
  ...
  async listRecentActivity(): Promise<readonly TodayActivityItem[]> {
    const [sessionRows, syncRows] = await Promise.all([
      this.database
        .select({
    ...
    const sessions: TodayActivityItem[] = sessionRows.map((session) => ({
      ...session,
      kind: "session" as const,
    }));
    const syncs: TodayActivityItem[] = syncRows.map((sync) => ({
      id: sync.id,
      kind: "sync" as const,
